...
export default function Sport() {
  ...
  const [screenSize, setScreenSize] = useState("sm"); // sm, md, lg
  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth < 640) {
        setScreenSize("sm");
        setExpanded(true); // sm -> expand true
      } else if (window.innerWidth < 1024) {
        setScreenSize("md");
        setExpanded(false); // md -> expand false
      } else {
        setScreenSize("lg");
        setExpanded(false); // lg -> expand false
      }
    };
