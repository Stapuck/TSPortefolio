...
const TooltipPoint = ({ 
  ...
  const themeClasses = {
    ...
    colored: {
      point: "bg-gradient-to-r from-orange-500 to-red-500 border-2 border-white shadow-lg",
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Price */}
        <div className={`font-bold ${currentSize.price} ${currentTheme.price} mb-1 text-center`}>
          {price}
        </div>

        {/* Description */}
        <div className={`${currentSize.text} ${currentTheme.text} text-center leading-tight`}>
          {text}
        </div>
        ...
        {variant === "click" && (
          <div className="mt-2 pt-2 border-t border-current border-opacity-20">
            <div className="flex justify-center">
              <div className={`w-2 h-2 rounded-full ${isVisible ? 'bg-current' : 'bg-current opacity-50'}`} />
            </div>
          </div>
        )}
      </div>
      ...
      {variant === "always" && (
        <div className="absolute -bottom-6 left-1/2 transform -translate-x-1/2 whitespace-nowrap">
          <div className={`text-xs px-2 py-1 rounded-full ${currentTheme.tooltip} ${currentTheme.text}`}>
